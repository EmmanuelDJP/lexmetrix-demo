...
export default function LexMetrixDemo() {
  ...
  return (
    // Contenedor principal: Altura exacta de la pantalla, sin scroll general
    <div className="h-screen w-full bg-slate-100 flex flex-col overflow-hidden font-sans text-slate-900">
      
      {/* Header Compacto */}
      <header className="flex-shrink-0 bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center z-10 shadow-sm">
        <div>
      ...
      <main className="flex-1 overflow-hidden p-4 md:p-6">
        <div className="h-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Columna 2 y 3: Visor de Datos (Derecha) */}
          <div className="col-span-1 lg:col-span-2 flex flex-col h-full gap-6 overflow-hidden">
            ...
            {/* Salida del Dictamen (Scroll Interno si el texto es muy largo) */}
              ...
              <div className="flex-1 overflow-y-auto p-0 md:p-6 bg-slate-50">
                ...
                {showResult && !isAuditing && (
                  <div className="animate-fade-in flex flex-col min-h-full">
                    <div className="bg-white border-l-4 border-slate-800 p-6 rounded-r-lg shadow-sm">
                      <pre className="whitespace-pre-wrap font-serif text-slate-800 text-sm md:text-base leading-relaxed">
                        {casoActivo.dictamen}
                      </pre>
                    </div>
                    
                    {/* Call To Action al final del scroll */}
                ...
                {!showResult && !isAuditing && (
                  <div className="h-full flex items-center justify-center text-slate-400 text-sm font-medium border-2 border-dashed border-slate-200 m-6 rounded-lg">
